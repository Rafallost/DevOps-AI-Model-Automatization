...
Workflow uruchamia się wyłącznie na push do gałęzi pasujących do wzorca \texttt{data/**}. Oznacza to, że kod przesyłany do \texttt{main} lub innych gałęzi nie inicjuje treningu --- pipeline startuje tylko gdy hook \texttt{pre-push} przekieruje dane na gałąź \texttt{data/TIMESTAMP}:
...
\begin{sloppypar}
Większość jobów uruchamia się na środowisku zarządzanym przez GitHub (\texttt{runs-on: ubuntu-latest}), co nie wymaga utrzymania własnej infrastruktury i nie generuje kosztów podczas oczekiwania. Wyjątkiem są joba \texttt{train} i \texttt{deploy} --- wykonują się na \texttt{runs-on: self-hosted}, czyli bezpośrednio na instancji EC2. Taki podział eliminuje transfer danych treningowych przez sieć publiczną: dane pozostają na EC2 przez cały czas treningu.
\end{sloppypar}
...
Bramka jakości jest mechanizmem decydującym, czy nowy model jest wystarczająco dobry, żeby trafić do produkcji. Implementacj korzysta z API MLflow Model Registry \cite{mlflow_model_registry} (\texttt{quality-gate.py}). Skrypt łączy się z MLflow przez \texttt{MlflowClient} i pobiera metryki aktualnego modelu Production:

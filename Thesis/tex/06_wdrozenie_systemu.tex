...
Infrastruktura uruchamiana jest jednorazowo poleceniem \texttt{terraform apply} (rys.~\ref{fig:terraform-apply}). Instancja EC2 jest następnie zatrzymywana i uruchamiana automatycznie przez workflow tylko na czas treningu i wdrożenia, co istotnie obniża koszty eksploatacji. Rysunki~\ref{fig:aws-ec2} i~\ref{fig:aws-s3} przedstawiają odpowiednio instancję EC2 i zasobniki S3 w konsoli AWS.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{img/06_wdrozenie_systemu/terraform_apply.png}
  \caption{Wynik polecenia \texttt{terraform apply} --- lista utworzonych zasobów AWS.}
...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\linewidth}
    ...
    \caption{Zasobniki S3: \texttt{wms-dvc-data-*} i \texttt{wms-mlflow-artifacts-*}.}
    \label{fig:aws-s3}
  \end{minipage}
\end{figure}
...
Konfiguracja instancji EC2 odbywa się automatycznie przy pierwszym uruchomieniu za pomocą skryptu \texttt{user-data}. Skrypt jest szablonowany przez Terraform i zawiera zmienne takie jak adres zasobnika S3 czy region.
...
Instancja korzysta z Amazon Linux 2023, który oferuje nowszy OpenSSL 3.0 i lepszy ekosystem pakietów niż poprzednia wersja Amazon Linux 2. Po zakończeniu \texttt{user-data} instancja jest gotowa do przyjęcia zadań z GitHub Actions bez dalszej konfiguracji ręcznej. Rysunek~\ref{fig:systemctl-status} potwierdza poprawne działanie usług systemd, a rysunek~\ref{fig:kubectl-pods} --- aktualny stan podów klastra k3s. Interfejs MLflow dostępny jest pod adresem \texttt{http://100.50.251.97:5000} (rys.~\ref{fig:mlflow-ui}).

\begin{figure}[H]
 \centering
 \begin{minipage}{0.48\linewidth}
   \includegraphics[width=\linewidth]{img/06_wdrozenie_systemu/systemctl_status.png}
   \caption{Stan usług \texttt{mlflow} i \texttt{actions.runner} w systemd.}
   \label{fig:systemctl-status}
 \end{minipage}\hfill
 \begin{minipage}{0.48\linewidth}
   \includegraphics[width=\linewidth]{img/06_wdrozenie_systemu/kubectl_pods.png}
   \caption{Pody klastra k3s --- \texttt{kubectl get pods -A}.}
...
\begin{figure}[H]
 ...
 \caption{Interfejs MLflow (\texttt{http://100.50.251.97:5000}) --- lista eksperymentów i runs z metrykami.}
 \label{fig:mlflow-ui}
\end{figure}
...
Po wdrożeniu API jest dostępne pod adresem \texttt{http://100.50.251.97:30080}. Dostępne endpointy to \texttt{/health} (status usługi), \texttt{/predict} (predykcja segmentacji) i \texttt{/metrics} (metryki Prometheus). Repozytorium ECR oraz odpowiedź endpointu \texttt{/health} przedstawiają rysunki~\ref{fig:ecr-repo} i~\ref{fig:api-health}.

\begin{figure}[H]
 \centering
 \begin{minipage}{0.48\linewidth}
   \includegraphics[width=\linewidth]{img/06_wdrozenie_systemu/ecr_repository.png}
...
Działanie stosu monitorowania ilustruje dashboard Grafana (rys.~\ref{fig:grafana-dashboard}) oraz widok Targets w Prometheus (rys.~\ref{fig:prometheus-targets}). Rysunek~\ref{fig:mlflow-registry} przedstawia rejestr modeli MLflow z aktualną wersją Production.
...
\begin{figure}[H]
 \centering
 \includegraphics[width=\linewidth]{img/06_wdrozenie_systemu/mlflow_registry.png}
 \caption{Rejestr modeli MLflow --- model \texttt{water-meter-segmentation} w stadium Production z metrykami \texttt{val\_dice} i \texttt{val\_iou}.}
...
\end{lstlisting}
  ...
  \item Uruchamia predykcję: \texttt{python WMS/src/predicts.py}
